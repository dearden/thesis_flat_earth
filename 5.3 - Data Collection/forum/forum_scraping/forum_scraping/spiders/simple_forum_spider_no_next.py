...
class SimpleForumScraper(scrapy.Spider):
    ...
    name = "simple_forum_scraper_no_next"
    ...
    def start_requests(self):
        ...
        urls = ["https://www.theflatearthsociety.org/forum/"]      # INSERT YOUR BOARD URL HERE
        for url in urls:
    ...
    def board_parse(self, response):
        ...
        # Find the number of the current page
        curr_page_num = int(response.xpath('//div[contains(@class,"pagelinks")]/strong/text()').get())

        # Find the url for the next page (link with the next number as text)
        next_page = response.xpath('//a[text()="{}"]/@href'.format(curr_page_num+1)).get()
